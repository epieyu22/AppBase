...
@Component({
    selector: 'professions-form',
    templateUrl: './professions-form.component.html'
  })
  export class ProfessionsFormComponent implements AfterViewInit {
    ...
    initData(isNew: boolean = false) {
      this.models$ = this.professionService.getAll();
  ...
  saveChanges() {
      ...
      this.professionService[this.isNew ? 'create' : 'update'](this.model)
            .subscribe(
                response => {
