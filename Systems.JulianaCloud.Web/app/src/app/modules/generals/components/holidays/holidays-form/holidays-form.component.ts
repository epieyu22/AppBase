...
@Component({
    selector: 'holidays-form',
    templateUrl: './holidays-form.component.html'
  })
  export class HolidaysFormComponent implements AfterViewInit {
    ...
    isNew: boolean
    translations: any = {}
    defaultValues = {key: 0, holidayDate: '', name: ''}
    ...
    initData(isNew: boolean = false) {
      this.models$ = this.holidayService.getAll();
  ...
  saveChanges() {
      ...
      this.holidayService[this.isNew ? 'create' : 'update'](this.model)
            .subscribe(
                response => {
