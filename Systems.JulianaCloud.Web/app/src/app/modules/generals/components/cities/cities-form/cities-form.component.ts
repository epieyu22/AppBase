...
@Component({
  ...
  export class CitiesFormComponent implements AfterViewInit {    
    @Input()
    model: any
    ...
    isNew: boolean
    translations: any = {}
    defaultValues = {key: 0, codDepto: '',codDane: '', name: ''}
    ...
    initData(isNew: boolean = false) {
      this.models$ = this.cityService.getAll();
  ...
  saveChanges() {
      this.spinner.show();
      this.cityService[this.isNew ? 'create' : 'update'](this.model)
            .subscribe(
                response => {
