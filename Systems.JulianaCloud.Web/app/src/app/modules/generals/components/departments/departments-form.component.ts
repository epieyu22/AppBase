...
@Component({
  ...
  export class DepartmentsFormComponent implements AfterViewInit {    
    @Input()
    model: any
    ...
    isNew: boolean
    translations: any = {}
    defaultValues = {key: 0, codDane: '', name: ''}
    ...
    initData(isNew: boolean = false) {
      this.models$ = this.departmentService.getAll();
  ...
  saveChanges() {
      this.spinner.show();
      this.departmentService[this.isNew ? 'create' : 'update'](this.model)
            .subscribe(
                response => {
