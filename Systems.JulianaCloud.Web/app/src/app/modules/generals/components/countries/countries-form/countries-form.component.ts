...
@Component({
  ...
  export class CountriesFormComponent implements AfterViewInit {
    ...
    initData(isNew: boolean = false) {
      this.models$ = this.countryService.getAll();
  ...
  saveChanges() {
      ...
      this.countryService[this.isNew ? 'create' : 'update'](this.model)
            .subscribe(
                response => {
