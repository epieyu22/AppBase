...
import { ArlFormComponent } from '@modules/generals/components/arl/arl-form/arl-form.component';
import { ArlService } from '@modules/generals/services/arl.service';
import { appendFile } from 'fs';

@Component({
  selector: 'app-arl-manager',
  templateUrl: './arl-manager.component.html',
  providers: [
    ArlService
  ]
})
export class ArlManagerComponent {
  self: any;
  popupVisible = false;
  currentModel: any
  data: any;
  ...
  collapsed = false;
  arlStore: CustomStore;
  gridContext: any;
  translations: any = {};
  @ViewChild("grid") dataGrid: DxDataGridComponent
  @ViewChild("form") form: ArlFormComponent
  ...
  constructor(
    ...
    this.arlStore = arlService.getStore();
      this.currentModel = {};
      this.self = this;
  ...
  getCrudConfig() {
    return {
      onEdit: (data: any) => this.arlService.update(data),
      onDelete: (data: any) => this.arlService.delete(data.code)
    }
  }
  ...
  deleteEntry(e) {
    let result = confirm(
      this.translations['GENERALS.ARL.CONFIRM_DELETE_MESSAGE']
  ...
  loadTranslations(){
    ...
    this.translateService.get([
      'GENERALS.ARL.CONFIRM_DELETE_MESSAGE',
