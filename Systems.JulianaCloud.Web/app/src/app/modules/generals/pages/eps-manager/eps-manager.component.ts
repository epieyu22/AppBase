...
import { EpsFormComponent } from '@modules/generals/components/eps/eps-form/eps-form.component';

@Component({
  selector: 'app-eps-manager',
  templateUrl: './eps-manager.component.html',
  providers: [
    EpsService
  ]
})
export class EpsManagerComponent {
  self: any;
  popupVisible = false;
  currentModel: any
  data: any;
  ...
  collapsed = false;
  epsStore: CustomStore;
  gridContext: any;
  translations: any = {};
  @ViewChild("grid") dataGrid: DxDataGridComponent
  @ViewChild("form") form: EpsFormComponent
  ...
  constructor(
    ...
    this.epsStore = epsService.getStore();
      this.currentModel = {};
      this.self = this;
  ...
  getCrudConfig() {
    return {
      onEdit: (data: any) => this.epsService.update(data),
      onDelete: (data: any) => this.epsService.delete(data.code)
    }
  }
  ...
  deleteEntry(e) {
    let result = confirm(
      this.translations['GENERALS.EPS.CONFIRM_DELETE_MESSAGE']
  ...
  loadTranslations(){
    ...
    this.translateService.get([
      'GENERALS.EPS.CONFIRM_DELETE_MESSAGE',
