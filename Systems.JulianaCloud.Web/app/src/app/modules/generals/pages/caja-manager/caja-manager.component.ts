...
import { CajasFormComponent } from '@modules/generals/components/cajas/cajas-form/cajas-form.component';

@Component({
  selector: 'app-caja-manager',
  templateUrl: './caja-manager.component.html',
  styleUrls: [],
  providers: [
    CajaService
  ]
})
export class CajaManagerComponent {
  self: any;
  popupVisible = false;
  currentModel: any
  data: any;
  ...
  collapsed = false;
  cajaStore: CustomStore;
  gridContext: any;
  translations: any = {};
  @ViewChild("grid") dataGrid: DxDataGridComponent
  @ViewChild("form") form: CajasFormComponent
  ...
  constructor(
    ...
    this.cajaStore = cajaService.getStore();
      this.currentModel = {};
      this.self = this;
  ...
  getCrudConfig() {
    return {
      onEdit: (data: any) => this.cajaService.update(data),
      onDelete: (data: any) => this.cajaService.delete(data.code)
    }
  }
  ...
  deleteEntry(e) {
    let result = confirm(
      this.translations['GENERALS.CAJAS.CONFIRM_DELETE_MESSAGE']
  ...
  loadTranslations(){
    ...
    this.translateService.get([
      'GENERALS.CAJAS.CONFIRM_DELETE_MESSAGE',
